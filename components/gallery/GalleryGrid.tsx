...
export default function GalleryGrid() {
  ...
  const slides = gridPhotos.map(p => ({ src: p.url }));
        ...
        {tabs.map(t => (
          <button
            ...
            className={`px-4 py-2 rounded-full font-semibold text-sm transition flex items-center gap-1.5 ${
              tab === t.key ? 'bg-[#E8640A] text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
            }`}
          >
            {t.label}
            {t.count !== undefined && (
              <span className={`text-xs px-1.5 py-0.5 rounded-full ${tab === t.key ? 'bg-white/25' : 'bg-gray-200 text-gray-500'}`}>
                {t.count}
              </span>
            )}
          </button>
      ...
      {tab === 'vergelijking' && (
        pairs.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {pairs.map((pair, i) => (
              <BeforeAfterSlider key={i} voorSrc={pair.voor.url} naSrc={pair.na.url} />
            ))}
          </div>
        )
      )}
      ...
      {tab !== 'vergelijking' && (
        gridPhotos.length === 0 ? (
          ...
          <>
              ...
              {gridPhotos.map((photo, i) => (
                ...
                >
                  ...
                  <div className={`absolute top-2 left-2 text-xs font-bold px-2 py-0.5 rounded-full ${
                    photo.category === 'voor' ? 'bg-blue-500/90 text-white' : 'bg-green-500/90 text-white'
                  }`}>
            ...
            <Lightbox open={open} close={() => setOpen(false)} index={index} slides={slides} on={{ view: ({ index: i }) => setIndex(i) }} />
          </>
        )
      )}
    </>
